...
2. **O `hl_sim` só substitui o que é hardware ou mundo.** Firmware do Booster
   ...
   Wi-Fi. Nada além disso. Precisamos dessa fidelidade para podermos alterar o brain validando estrategias de jogo que desenvolveremos no futuro
...
HLRoboSim/
  ...
  │   ├── launch/
  │   │   └── robot.launch.py # = start.sh do robocup_demo sem vision/whistle + emulator.launch.py
  ...
  ├── src/
  │   ├── scenes/
  │   ├── assets/ # MuJoCo (campo 14×9, bola, robotN/*.xml) — como hoje
  │   ├── config/
  ...
  │       │   ├── base.py
  │       │   ├── zombie.py # agente sem o brain, 
  │       │   └── player.py # jogador principal com o brain
